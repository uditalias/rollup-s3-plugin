...
class S3Plugin {
  ...
  getAssetFiles() {
    ...
    for (const name in this.bundle) {
      path = this.bundle[name].facadeModuleId;
...
module.exports = function RollupS3Plugin(options) {
  return {
    name: "rollup-s3-plugin",
    writeBundle: (bundle) => {
      const plugin = new S3Plugin(bundle, options);
